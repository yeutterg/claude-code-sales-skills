...
Determine the repo path by finding where this skill file lives. The repo root is the parent directory of the `sales-setup/` folder — e.g., if this skill is at `~/repos/claude-code-obsidian-commands/sales-setup/SKILL.md`, the repo root is `~/repos/claude-code-obsidian-commands/`.
